  ...
  useEffect(() => {
    ...
    const fetchProfileData = async () => {
      try {
        ...
        // Fetch log stories
        try {
          const { data: logStoriesData, error: logStoriesError } =
            await getUserLogStories(data.id);
          if (logStoriesError) {
            console.error(logStoriesError);
          } else if (logStoriesData) {
            setLogStories(logStoriesData);
          }
        } catch (error) {
          console.error("Error fetching log stories", error);
        } finally {
          setLogStoriesLoading(false);
        }

        // Fetch brand connects
        try {
