...
interface ChatProps {
  ...
  onReplyClick?: (comment: ChatMessagesDTO) => void;
  isReplying?: boolean;
}
      ...
      <div className={`flex items-start gap-4 `}>
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <Link
              href={`/user-profile/${comment.user_info.username}`}
          ...
          <div
            className={`${
              profile?.id === comment.user_id ? "bg-[#dbeafe]" : "bg-gray-100"
            } rounded-2xl rounded-tl-none p-4 max-w-xl mb-2`}
          >
          ...
          {isReply ? null : (
            <div className="flex items-center gap-6 ml-2">
              ...
              {/* Chat backs count toggle */}
              <button
                onClick={toggleReplies}
                className="flex items-center gap-2 font-bold text-green-600 hover:text-green-700"
              >
                {comment.chat_back_count > 0 && (
                  <span className="text-sm font-bold text-[#f7a01c]">
                    {comment.chat_back_count} chat backs
                  </span>
                )}
              </button>
            </div>
...
export function SystemChat({
  ...
  comment: Partial<ChatMessagesDTO>;
  ...
  return (
    <div
      className={`px-4 py-2 rounded-lg border border-1 text-center ${
        colors[type || "info"]
      }`}
    >
